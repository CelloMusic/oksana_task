...
public class ObratAbel {
	...
	private double Abel_i(double r) {
		...
		for (int i = 1; i <= this.n; i++) {
			...
			I1r += this.C[i].ampl() * k * Bessel.BesselFunc(k * r, 0) / 4;
		}
		return I1r;
	}
	...
	public double Abel(double r) {
		double J1r = Abel_i(r);
		double A = Math.sqrt(Math.pow(this.C[0].ampl() / (2 * Math.PI * J1r), 2) + this.R * this.R);
		double J0r = this.C[0].ampl() / (2 * Math.PI * Math.sqrt(A*A - this.R*this.R));
		return J0r + J1r;
	}
